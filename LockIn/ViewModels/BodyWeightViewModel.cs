...
public partial class BodyWeightViewModel(DatabaseService db) : ObservableObject
{
    ...
    public BodyWeightDrawable ChartDrawable { get; } = new();
    ...
    private async Task LogWeightAsync()
    {
        var result = await Shell.Current.DisplayPromptAsync(
            "Logga vikt",
            "Ange din kroppsvikt i kg:",
            keyboard: Keyboard.Numeric,
            placeholder: "t.ex. 80.5");
        ...
        var entry = new BodyWeightEntry { LoggedAt = DateTime.Now, WeightKg = kg };
        await db.SaveBodyWeightEntryAsync(entry);
        await LoadAsync();
    }
